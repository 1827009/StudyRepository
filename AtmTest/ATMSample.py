...
class AtmUi:
    ...
    def createGUI(self):
        drawerButton = tk.Button(
            self.frame, text="引出", command=self.drawer, font=(",14"))
        drawerButton.grid(row=2, column=5)
        depositButton = tk.Button(
            self.frame, text="預入", command=self.deposit, font=(",14"))
        depositButton.grid(row=3, column=5)
        depositButton = tk.Button(
            self.frame, text="記帳", command=self.printing, font=(",14"))
        depositButton.grid(row=4, column=5)

        btList = []
        for i in range(10):
            btList.append(tk.Button(self.frame, text=i, command=lambda x=i: self.key(
                x), font=('Helvetica', 14), width=2, bg='#ffffc0'))
        ...
        depositButton = tk.Button(self.frame, text="C", command=self.clear, font=(
            'Helvetica', 14), width=2, bg='#ffffc0')
    ...
    def passbookDraw(self):
        self.passbookUi = tk.Label(
            self.frame, text="通帳："+str(self.business.passbook.text))
    ...
    def moneyDraw(self):
        self.moneytext = tk.Label(
            self.frame, text="残高："+str(self.business.account.money))
    ...
    def cashsDraw(self):
        ...
        for i in range(len(self.business.cashs.moneys)):
            casshtext += str(self.business.cashs.moneys[i].worth)+"円: "+str(
                self.business.cashs.moneys[i].quantity)+"枚\n"
